...
def read_raster_cell_value(input_rasterfile,cell_x_y):
	dataset=gdal.Open(input_rasterfile)
	cell_value=dataset.ReadAsArray(cell_x_y[0],cell_x_y[1],1,1)
	cell_value_setnull=np.array(cell_value)
	cell_value_setnull[cell_value_setnull==-9999.0]=np.nan
	return cell_value_setnull

def read_raster_ts_cell(img_path_list,cell_x_y):
	cell_date=[]
	cell_value=[]
	for i in img_path_list:
		cell_date.append(i.split('/')[-1][:8])
		cell_value.append(read_raster_cell_value(i,cell_x_y)[0])
		print('progress:'+str(round((img_path_list.index(i)+1.0)*100.0/len(img_path_list),3))+'%')
	cell_value_setnull=np.array(cell_value).reshape(-1)
	cell_value_setnull[cell_value_setnull==-9999.0]=np.nan
	return pd.to_datetime(cell_date,format='%Y%m%d'),cell_value_setnull
...
def get_month_max(daily_data,pd_time,year):
	month_max_value=[]
	max_v_moth_time=[]
	for m in range(1,13):
		this_m_value=daily_data[(pd_time.year==year) & (pd_time.month==m)]
		this_m_time=pd_time[(pd_time.year==year) & (pd_time.month==m)]
		month_max_value.append(np.nanmax(this_m_value))
		max_v_moth_time.append(this_m_time[this_m_value==np.nanmax(this_m_value)])
	return month_max_value,max_v_moth_time


def get_inter_monmax_similarity(daily_data,pd_time,year,threshold):
	month_all_max_value=[]
	all_max_v_moth_time=[]
	for m in range(1,13):
		#当月所有值
		...
		if m<12:
			...
			#获取下个月最大的数据
			next_m_value=daily_data[(pd_time.year==year) & (pd_time.month==m+1)]
			next_m_time=pd_time[(pd_time.year==year) & (pd_time.month==m+1)]
			next_m_max_value=np.nanmax(next_m_value)
			#计算相邻接两个月最大值的均值
			this_next_m_max_value_mean=np.mean([this_m_max_value,next_m_max_value])
			#计算两者之间的小值
			this_next_m_max_value_min=np.min([this_m_max_value,next_m_max_value])
			#获取与每个月最大值相近的数据和时间		
		...
		if m==12:
			...
			this_m_max_value=np.nanmax(this_m_value)
			#获取上个月最大的数据
			last_m_value=daily_data[(pd_time.year==year) & (pd_time.month==m-1)]
			last_m_time=pd_time[(pd_time.year==year) & (pd_time.month==m-1)]
			last_m_max_value=np.nanmax(next_m_value)
			#计算相邻接两个月最大值的均值
			this_last_m_max_value_mean=np.mean([this_m_max_value,last_m_max_value])
			#计算两者之间的小值
			this_last_m_max_value_min=np.min([this_m_max_value,last_m_max_value])
			#获取与每个月最大值相近的数据和时间		
			month_all_max_value=np.hstack([month_all_max_value,this_m_value[(this_m_value/this_last_m_max_value_mean>=threshold) | (this_m_value/this_last_m_max_value_min>=threshold)].tolist()])
			all_max_v_moth_time=np.hstack([all_max_v_moth_time,this_m_time[(this_m_value/this_last_m_max_value_mean>=threshold) | (this_m_value/this_last_m_max_value_min>=threshold)].tolist()])
	return month_all_max_value,all_max_v_moth_time


def select_right_month_data(this_m_value,this_m_time):
	#新建一个dataframe
	DF_mon_data=pd.DataFrame(data=this_m_value,index=this_m_time,columns=['NDVI'])
	#排序
	sort_DF_mon_data=DF_mon_data.sort_values(by='NDVI',ascending=False)
	if sort_DF_mon_data.iloc[0]['NDVI']/sort_DF_mon_data.iloc[1]['NDVI']>1.3:
		DF_mon_data.drop(index=sort_DF_mon_data.iloc[0].name,inplace=True)
	return np.array(DF_mon_data['NDVI']),DF_mon_data.index

@jit()
def get_all_usefull_data(daily_data,pd_time,threshold):
	month_all_max_value=[]
	all_max_v_moth_time=[]
	first_year=pd_time[0].year
	last_year=pd_time[-1].year
	first_year_month=pd_time[0].month
	last_year_month=pd_time[-1].month
	for year in range(first_year,last_year+1):
		...
		for m in range(start_mon,end_mon+1):   
			ini_this_m_value=daily_data[(pd_time.year==year) & (pd_time.month==m)]
			ini_this_m_time=pd_time[(pd_time.year==year) & (pd_time.month==m)]
			this_m_value,this_m_time=select_right_month_data(ini_this_m_value,ini_this_m_time)
			this_m_max_value=np.nanmax(this_m_value)
			if year!=first_year and year != last_year:
				if m>1 and m<12:
					...
					next_m_max_value=np.nanmax(next_m_value)
					#上个月最大值
					last_m_value=daily_data[(pd_time.year==year-1) & (pd_time.month==12)]
					last_m_time=pd_time[(pd_time.year==year-1) & (pd_time.month==12)]
					last_m_max_value=np.nanmax(last_m_value)	
				elif m==12:
					next_m_value=daily_data[(pd_time.year==year+1) & (pd_time.month==1)]
					next_m_time=pd_time[(pd_time.year==year+1) & (pd_time.month==1)]
					next_m_max_value=np.nanmax(next_m_value)
					last_m_value=daily_data[(pd_time.year==year) & (pd_time.month==m-1)]
					last_m_time=pd_time[(pd_time.year==year) & (pd_time.month==m-1)]
					last_m_max_value=np.nanmax(last_m_value)
		...
		usefull_ndvi_time_DF=pd.DataFrame(data=month_all_max_value,index=all_max_v_moth_time,columns=['NDVI'])
		sorted_usefull_ndvi_time_DF=usefull_ndvi_time_DF.sort_values(by='NDVI',ascending=False)
		mean_ndvi_2_10=np.mean(sorted_usefull_ndvi_time_DF.iloc[np.arange(1,11)]['NDVI'])
		if (sorted_usefull_ndvi_time_DF.iloc[0]['NDVI']-mean_ndvi_2_10)/mean_ndvi_2_10>0.4:
			drop_index=sorted_usefull_ndvi_time_DF.iloc[0].name
			usefull_ndvi_time_DF.drop(axis=0,index=drop_index,inplace=True)
	return usefull_ndvi_time_DF[usefull_ndvi_time_DF['NDVI']>0]

def select_right_month_data_list_time(this_m_value,this_m_time):
	sorted_index=np.argsort(this_m_value)
	if this_m_value[sorted_index[-1]]/this_m_value[sorted_index[-2]]>1.3:
		this_m_value=np.delete(this_m_value,sorted_index[-1])
		this_m_time=np.delete(this_m_time,sorted_index[-1])
	return this_m_value,this_m_time
...
def get_all_usefull_data_jit(daily_data,pd_time,threshold):
	...
	if sorted_usefull_ndvi_time_DF.shape[0]>5:
		...
		pass
	# If the maximum value in the entire time series is 39% greater than the mean of adjacent 9 or 19 data points, delete this outlier.
...
def spline_interpolation_data(pd_time,usefull_ndvi_time_DF):
	useless_time=np.setdiff1d(pd_time,usefull_ndvi_time_DF.index)  
	useless_time_index=np.array([np.argwhere(pd_time==i)[0,0] for i in useless_time])+1
	usefull_time_index=np.array([np.argwhere(pd_time==i)[0,0] for i in usefull_ndvi_time_DF.index])+1
	intpol_fun = interp1d(usefull_time_index,usefull_ndvi_time_DF.values.reshape(-1),kind="quadratic",bounds_error=False,fill_value=np.nan)
	useless_time_spline_values=intpol_fun(useless_time_index)
	all_data=np.full(pd_time.shape[0],fill_value=-1.0)
	all_data[useless_time_index-1]=useless_time_spline_values
	all_data[usefull_time_index-1]=usefull_ndvi_time_DF.values.reshape(-1)
	return pd.DataFrame(index=pd_time,data=all_data,columns=['NDVI'])


def fit_data_with_scipy(pd_time,usefull_ndvi_time_DF):
	useless_time=np.setdiff1d(pd_time,usefull_ndvi_time_DF.index)  
	useless_time_index=np.array([np.argwhere(pd_time==i)[0,0] for i in useless_time])+1
	usefull_time_index=np.array([np.argwhere(pd_time==i)[0,0] for i in usefull_ndvi_time_DF.index])+1
	params0 = [0.0, 0.0, 0.0,0.0] 
	res = minimize(fun=loss_function, x0=params0, args=(usefull_time_index,usefull_ndvi_time_DF),method='Powell')  
	params = res.x
	predict_function = lambda x: params[0] +  params[1]*x+ params[2]*np.sin((x/180.0) * np.pi+params[3] )  
	useless_time_spline_values = predict_function(useless_time_index)  
	usefull_time_spline_values = predict_function(usefull_time_index)  
	all_data=np.full(pd_time.shape[0],fill_value=-1.0)
	all_data[useless_time_index-1]=useless_time_spline_values
	all_data[usefull_time_index-1]=usefull_time_spline_values
	return pd.DataFrame(index=pd_time,data=all_data,columns=['NDVI'])


def loss_function(params):
	params0 = [0.0, 0.0, 0.0,0.0]  
	real_x = usefull_time_index
	real_y = usefull_ndvi_time_DF.values.reshape(-1)
	small_function = lambda x: params[0] +  params[1]*x+ params[2]*np.sin((x/180.0) * np.pi+params[3] )
	predict = small_function(real_x)
	result = np.mean((predict - real_y) ** 2)
	return result

def fit_data_with_scipy_year_by_year(pd_time,usefull_ndvi_time_DF):
	all_data=np.full(pd_time.shape[0],fill_value=-1.0)
	all_years=np.unique(usefull_ndvi_time_DF.index.year)
	for y in all_years:
		if y>all_years[0] and y<all_years[-1]:
			this_year_usefull_data_DF=usefull_ndvi_time_DF[(usefull_ndvi_time_DF.index.year==y) | ((usefull_ndvi_time_DF.index.year==y-1) & (usefull_ndvi_time_DF.index.month>=9)) | ((usefull_ndvi_time_DF.index.year==y+1) & (usefull_ndvi_time_DF.index.month<=4))]
		elif y==all_years[0]:
			this_year_usefull_data_DF=usefull_ndvi_time_DF[(usefull_ndvi_time_DF.index.year==y) | ((usefull_ndvi_time_DF.index.year==y+1) & (usefull_ndvi_time_DF.index.month<=4))]
		else:
			this_year_usefull_data_DF=usefull_ndvi_time_DF[(usefull_ndvi_time_DF.index.year==y) | ((usefull_ndvi_time_DF.index.year==y-1) & (usefull_ndvi_time_DF.index.month>=9))]
		...
		all_data[this_year_all_data_time_index-1]=fit_this_year_data_piecewise_fitting(this_year_usefull_time_index,this_year_usefull_data_DF,this_year_all_data_time,this_year_all_data_time_index)
	return pd.DataFrame(index=pd_time,data=all_data,columns=['NDVI'])


def fit_this_year_data(this_year_usefull_time_index,this_year_usefull_data_DF,this_year_all_data_time,this_year_all_data_time_index):
	params0 = [0.0, 0.0, 0.0,0.0]  
	res = minimize(fun=loss_function_year_by_year, x0=params0, args=(this_year_usefull_time_index,this_year_usefull_data_DF),method='Powell')  
	params = res.x
	predict_function = lambda x: params[0] +  params[1]*x+ params[2]*np.sin((x/180.0) * np.pi+params[3] ) 
	this_year_all_spline_values = predict_function(this_year_all_data_time_index)
	return this_year_all_spline_values


def loss_function_year_by_year(params,this_year_usefull_time_index,this_year_usefull_data_DF):
	params0 = [0.0, 0.0, 0.0,0.0] 
	real_x = this_year_usefull_time_index
	real_y = this_year_usefull_data_DF.values.reshape(-1)
	small_function = lambda x: params[0] +  params[1]*x+ params[2]*np.sin((x/180.0) * np.pi+params[3] )
	predict = small_function(real_x)
	result = np.mean((predict - real_y) ** 2)
	return result

def fit_data_with_annual_piecewise_fitting(pd_time,usefull_ndvi_time_DF):
	all_data=np.full(pd_time.shape[0],fill_value=-1.0)
	all_years=np.unique(usefull_ndvi_time_DF.index.year)
	for y in all_years:
		if y>all_years[0] and y<all_years[-1]:
		...
		this_year_all_data_time=pd_time[pd_time.year==y]
		this_year_all_data_time_index=np.array([np.argwhere(pd_time==i)[0,0] for i in this_year_all_data_time])+1
		all_data[this_year_all_data_time_index-1]=fit_this_year_data_piecewise_fitting(y,this_year_usefull_time_index,this_year_usefull_data_DF,this_year_all_data_time_index)
	return pd.DataFrame(index=pd_time,data=all_data,columns=['NDVI'])


def fit_this_year_data_piecewise_fitting(y,this_year_usefull_time_index,this_year_usefull_data_DF,this_year_all_data_time_index):
	if y==1981 or y==2023:
		params_all_0 = [0.0, 0.0, 0.0,0.0]
		res_all = minimize(fun=loss_function_piecewise_fitting_all_time, x0=params_all_0, args=(this_year_usefull_time_index,this_year_usefull_data_DF),method='Powell')  # nelder-mead powell
		params_all = res_all.x
		predict_function = lambda x: params_all[0] +  params_all[1]*x+ params_all[2]*np.sin((x/180.0) * np.pi+params_all[3])  # 这个是我们要构造的目标函数
		this_year_all_spline_values = predict_function(this_year_all_data_time_index)  # 计算预测值
	else:
		if this_year_usefull_data_DF['NDVI'].idxmax().year==y:
			...
			params_first_0 = [0.0, 0.0, 0.0,0.0]  # 设置一个初始化参数，用来等待模拟。第一段数据
			res_first = minimize(fun=loss_function_piecewise_fitting_first, x0=params_first_0, args=(first_this_year_usefull_time_index,first_this_year_usefull_data_DF),method='Powell')  # nelder-mead powell
			params_first = res_first.x
			predict_function_first = lambda x: params_first[0] +  params_first[1]*x+ params_first[2]*np.sin((x/180.0) * np.pi+params_first[3])  # 这个是我们要构造的目标函数
			first_this_year_all_spline_values = predict_function_first(first_this_year_all_data_time_index)  # 计算预测值
			params_second_0 = [0.0, 0.0, 0.0,0.0]  # 设置一个初始化参数，用来等待模拟。第一段数据
			res_second = minimize(fun=loss_function_piecewise_fitting_second, x0=params_second_0, args=(second_this_year_usefull_time_index,second_this_year_usefull_data_DF),method='Powell')  # nelder-mead powell
			params_second = res_second.x
			predict_function_second = lambda x: params_second[0] +  params_second[1]*x+ params_second[2]*np.sin((x/180.0) * np.pi+params_second[3])   # 这个是我们要构造的目标函数
			second_this_year_all_spline_values = predict_function_second(second_this_year_all_data_time_index)  # 计算预测值
			...
			if this_year_usefull_time_index[min_value_index+9]>this_year_all_data_time_index[-1]:
				params_all_0 = [0.0, 0.0, 0.0,0.0]  # 设置一个初始化参数，用来等待模拟。第一段数据
				res_all = minimize(fun=loss_function_piecewise_fitting_all_time, x0=params_all_0, args=(this_year_usefull_time_index,this_year_usefull_data_DF),method='Powell')  # nelder-mead powell
				params_all = res_all.x
				predict_function = lambda x: params_all[0] +  params_all[1]*x+ params_all[2]*np.sin((x/180.0) * np.pi+params_all[3])  # 这个是我们要构造的目标函数
				this_year_all_spline_values = predict_function(this_year_all_data_time_index)  # 计算预测值
				...
				first_polyfit_fun=np.polyfit(first_this_year_usefull_time_index, first_this_year_usefull_data_DF.values.reshape(-1), 5)
				first_this_year_all_spline_values = np.polyval(first_polyfit_fun,first_this_year_all_data_time_index)
				second_polyfit_fun=np.polyfit(second_this_year_usefull_time_index, second_this_year_usefull_data_DF.values.reshape(-1), 5)
				second_this_year_all_spline_values = np.polyval(second_polyfit_fun,second_this_year_all_data_time_index)
				overlap_sum_num=first_overlap_num+second_overlap_num
				this_year_all_spline_values=np.hstack([first_this_year_all_spline_values[:-overlap_sum_num],np.mean([first_this_year_all_spline_values[-overlap_sum_num:],second_this_year_all_spline_values[:overlap_sum_num]],axis=0),second_this_year_all_spline_values[overlap_sum_num:]])
		else:
			polyfit_fun=np.polyfit(this_year_usefull_time_index, this_year_usefull_data_DF.values.reshape(-1), 5)
			this_year_all_spline_values = np.polyval(polyfit_fun,this_year_all_data_time_index)
	return this_year_all_spline_values

def loss_function_piecewise_fitting_first(params_first,first_this_year_usefull_time_index,first_this_year_usefull_data_DF):
	real_x = first_this_year_usefull_time_index
	real_y = first_this_year_usefull_data_DF.values.reshape(-1)
	small_function =lambda x: params_first[0] +  params_first[1]*x+ params_first[2]*np.sin((x/180.0) * np.pi+params_first[3])
	predict = small_function(real_x)
	result = np.mean((predict - real_y) ** 2)
	return result


def loss_function_piecewise_fitting_second(params_second,second_this_year_usefull_time_index,second_this_year_usefull_data_DF):
	real_x = second_this_year_usefull_time_index
	real_y = second_this_year_usefull_data_DF.values.reshape(-1)
	small_function = lambda x: params_second[0] +  params_second[1]*x+ params_second[2]*np.sin((x/180.0) * np.pi+params_second[3])
	predict = small_function(real_x)
	result = np.mean((predict - real_y) ** 2)
	return result

def loss_function_piecewise_fitting_all_time(params,this_year_usefull_time_index,this_year_usefull_data_DF):
	real_x = this_year_usefull_time_index
	real_y = this_year_usefull_data_DF.values.reshape(-1)
	small_function = lambda x: params[0] +  params[1]*x+ params[2]*np.sin((x/180.0) * np.pi+params[3])
	predict = small_function(real_x)
...
def fit_line_all_column_data(txt_filepath):
	all_start_time=time.time()
	threshold=0.8
	save_txt_filepath="E:/Fit_line_txt/"
	...
	if max_column_num==3:
		...
		this_line_save_data=np.full([pd_time.shape[0],7202],'n').astype('U11')
		...
		starttime=time.time()
		for c in range(this_line_all_column_array.shape[1]):
			starttime1=time.time()
			if np.isnan(this_line_all_column_array[:,c]).sum()==this_line_all_column_array.shape[0]:
				...
				if np.isnan(this_line_all_column_array[:,c]).sum()/this_line_all_column_array.shape[0]>0.3:
					...
					this_line_save_data[:,c+2]=np.round(this_line_fitted_data,decimals=3)
			endtime2=time.time()
			get_processing_fiting(line_number,c,this_line_all_column_array.shape[1],starttime,endtime2)
		write_2dim_list_to_txt_file(save_file,this_line_save_data.tolist())
		all_endtime=time.time()
		print(str(line_number)+'].txt done: '+str(round((all_endtime-all_start_time)/60.0,4))+"  min")
